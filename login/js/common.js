...
$("#login_input4 input").on('click',function(){//登陆按钮
  ...
  var password = $("#login_input12").val();
     if(user==''||user=='用户名'){
		...
		$.ajax({
			type: 			"POST",
			url: 			"http://120.77.245.158:8088/api/User/Login",
			...
			success:function(json){
				if(json.ID > 0){
					document.getElementById("welcome_user").innerHTML = json.UserName;
					//window.location.href="http://120.77.245.158/bookstore";
					$("#login_content_log_02").fadeOut();
					$("#login_content_log_03").fadeIn();
					sessionStorage.setItem('UserCode', json.ID);
					//alert(getCookie(user));
					var t1 = setTimeout("window.location.href='http://120.77.245.158/bookshelf.shtml'",1000);
				}
				else{
